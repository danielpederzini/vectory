...
class KafkaOutboxRelayTest {
    ...
    void shouldPublishPendingEventsAndMarkThemPublished() {
        OutboxEventEntity event = pendingEvent(OutboxTopics.USERS_CREATED, "the-key");
        when(outboxRepository.findByPublicationInstantIsNullOrderByCreationInstantAsc(any(Limit.class)))
                .thenReturn(List.of(event));
        when(kafkaTemplate.send(OutboxTopics.USERS_CREATED, "the-key", event.getPayload().toString()))
                .thenReturn(CompletableFuture.completedFuture(null));

        relay.publishPending();

        verify(kafkaTemplate).send(OutboxTopics.USERS_CREATED, "the-key", event.getPayload().toString());
    ...
    void shouldLeaveEventUnpublishedWhenSendFails() {
        OutboxEventEntity event = pendingEvent(OutboxTopics.USERS_CREATED, "key");
        when(outboxRepository.findByPublicationInstantIsNullOrderByCreationInstantAsc(any(Limit.class)))
                .thenReturn(List.of(event));
